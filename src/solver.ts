...
import { PackageDomain, Solution } from "./types";
import { satisfiesRange } from "./utils/semverUtils";
import { GREEN, RED, RESET } from "./utils/colors";

type Z3Bool = Bool<"main">;

export const solvePeerDependencies = async (
  ...
  const variables = new Map<string, Z3Bool>();
  ...
  const getVar = (varName: string): Z3Bool => {
    const v = variables.get(varName);
    if (v === undefined) throw new Error(`Bug: missing Z3 variable for ${varName}`);
    return v;
  };
  ...
  for (const domain of domains) {
    const candidateVars = domain.candidates.map((c) =>
      getVar(`${c.packageName}@${c.version}`),
    );

    if (candidateVars.length === 0) {
      console.error(`  ${RED}✗ No candidates for ${domain.packageName}${RESET}`);
      return null;
    }
  ...
  for (const domain of domains) {
    for (const candidate of domain.candidates) {
      ...
      for (const [peerName, peerRange] of Object.entries(candidate.peerDependencies)) {
        ...
        if (!peerDomain) continue; // peer not in our dependency graph

        const compatiblePeerVars = peerDomain.candidates
          .filter((pc) => satisfiesRange(pc.version, peerRange))
          .map((pc) => variables.get(`${pc.packageName}@${pc.version}`))
          .filter((v): v is Z3Bool => v !== undefined);
        ...
        if (compatiblePeerVars.length === 0) {
          ...
          console.log(
            `  WARN: ${candidate.packageName}@${candidate.version} requires ${peerName}${peerRange}, but no compatible version exists`,
          );
  ...
  if (result === "sat") {
    console.log(`  ${GREEN}SAT - Solution found!${RESET}\n`);
  ...
  console.log(`  ${RED}UNSAT - No solution found${RESET}\n`);
  return null;
};
